  ...
  return (
    <div className="max-w-xl mx-auto">
      ...
      <p className="text-sm text-gray-500 mb-4">
        Words in this list are blocked from custom poll entries. Whole-word, case-insensitive matching.
      </p>
      ...
      <div className="flex gap-2 mb-4">
        ...
        />
        <button onClick={addWord} className="px-4 py-2 bg-purple-600 text-white rounded text-sm hover:bg-purple-700">Add</button>
      </div>
      ...
      {words.length === 0 ? (
        ...
        <Card>
          <div className="space-y-1">
            {words.map(w => (
              <div key={w.id} className="flex justify-between items-center text-sm py-1 px-2 rounded hover:bg-gray-50">
                <span className="font-mono text-red-700">{w.word}</span>
                <button onClick={() => deleteWord(w.id)} className="text-red-500 text-xs hover:underline">Remove</button>
              </div>
            ))}
          </div>
        </Card>
