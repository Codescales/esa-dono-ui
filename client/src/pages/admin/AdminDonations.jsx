    ...
    <div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full bg-white rounded shadow text-sm">
            <thead className="bg-gray-100">
              <tr>{['Donor','Email','Amount','Comment','Date'].map(h => <th key={h} className="text-left px-4 py-2">{h}</th>)}</tr>
            </thead>
            <tbody>
              {donations.map(d => (
                <tr key={d.id} className="border-t">
                  ...
                  <td className="px-4 py-2 text-gray-400">{new Date(d.created_at).toLocaleDateString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {tab === 'claims' && (
        <div className="overflow-x-auto">
          <table className="w-full bg-white rounded shadow text-sm">
            <thead className="bg-gray-100">
              <tr>{['Donor','Reward','Status','Data','Date','Action'].map(h => <th key={h} className="text-left px-4 py-2">{h}</th>)}</tr>
            </thead>
            <tbody>
              {claims.map(c => (
                <tr key={c.id} className="border-t">
                  ...
                  <td className="px-4 py-2">
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${c.status === 'FULFILLED' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                      {c.status}
                    </span>
                  </td>
