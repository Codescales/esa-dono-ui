...
const EMPTY = { title: '', description: '', type: 'DIGITAL', cost_cents: 100, quantity_total: '', is_active: true, custom_type_label: '' };
...
export default function AdminRewards() {
  ...
  const openEdit = r => { setForm({ ...r, cost_cents: r.cost_cents, quantity_total: r.quantity_total ?? '' }); setModal(r); setError(''); };
      ...
      <div className="overflow-x-auto">
        <table className="w-full bg-white rounded shadow text-sm">
          <thead className="bg-gray-100">
            <tr>{['Title','Type','Cost','Qty','Active','Actions'].map(h => <th key={h} className="text-left px-4 py-2">{h}</th>)}</tr>
          </thead>
          <tbody>
            {rewards.map(r => (
              <tr key={r.id} className="border-t">
                <td className="px-4 py-2 font-medium">{r.title}</td>
                <td className="px-4 py-2"><span className="bg-purple-100 text-purple-700 px-2 py-0.5 rounded text-xs">{r.type}</span></td>
                <td className="px-4 py-2">{fmt(r.cost_cents)}</td>
                <td className="px-4 py-2">{r.quantity_total ?? '∞'} ({r.quantity_claimed} claimed)</td>
        ...
        <Modal title={modal === 'create' ? 'New Reward' : 'Edit Reward'} onClose={() => setModal(null)}>
          ...
          <div className="mb-3">
            ...
            <select className="w-full border rounded px-3 py-2 text-sm" value={form.type} onChange={e => setForm(d => ({ ...d, type: e.target.value }))}>
              {['DIGITAL','PHYSICAL','SHOUTOUT','CUSTOM'].map(t => <option key={t}>{t}</option>)}
            </select>
          </div>
          <div className="mb-3 flex items-center gap-2">
            <input type="checkbox" id="is_active" checked={form.is_active} onChange={e => setForm(d => ({ ...d, is_active: e.target.checked }))} />
