...
A `Stream` model (`id`, `name`, `is_active`) lets one deployment run multiple concurrent donation streams/campaigns (e.g. two simultaneous runs) while routing each donation to the correct overlay/stream. Key invariants:

- **Every donation is required to route to exactly one stream.** `POST /api/pledge` rejects a missing/unknown/inactive `stream_id` with 400 — this is not optional, since downstream overlays key off it.
- **Incentives are either stream-specific or shared.** `Reward`, `Poll`, and `FundGoal` each have a nullable `stream_id`: `null` means the incentive is shared and shows up (and can be added to the cart) regardless of which stream is selected; a set value scopes it to that one stream.
- **Incentives cannot be mixed across streams in a single donation.** `createPledge()` rejects any cart item whose incentive `stream_id` is set and differs from the pledge's `stream_id` (`"... belongs to a different stream and cannot be added to this cart"`). Shared incentives are always allowed.
- **The stream propagates from pledge to donation.** `processDonation()` copies `pledge.stream_id` onto the created `Donation` on fulfillment (`server/services/donation.ts`); non-pledge donations (e.g. `POST /api/admin/simulate-donation`) can pass `stream_id` directly via the `streamId` option.
- **Streams are admin/moderator-managed**, not env-configured — `GET/POST /api/admin/streams`, `PUT/DELETE /api/admin/streams/:id` (mirrored under `/api/moderator/streams`). Deleting a stream **deactivates** it (`is_active: false`) rather than removing the row, since incentives/donations/pledges may still reference it.
- **Per-stream totals are admin-only.** `GET /api/admin/stats` includes a `streams: [{ id, name, raised_cents, donations }]` breakdown; the public `/api/campaign` endpoint intentionally keeps a single overall raised/goal total — there is no public per-stream leaderboard.
- **Client**: the `/donate` page requires selecting a stream (via `CartContext.selectStream`) before showing any incentives — the reward/poll/goal lists returned by the context are pre-filtered to shared + the selected stream. Switching streams with stream-specific items already in the cart triggers a confirm dialog (`CartContext.pendingStreamId` / `confirmStreamSwitch` / `cancelStreamSwitch`) that drops those items on confirmation and keeps shared ones — the "no mixing incentives across streams" rule enforced client-side before the server ever sees the request.
